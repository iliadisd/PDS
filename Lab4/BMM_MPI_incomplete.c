...
uint32_t *Multiply(uint32_t *coo_row, uint32_t *coo_col, uint32_t n, uint32_t nz){
  ...
  double t1, t2;
  int p,id,workers,source,dest,rows,offset;
  MPI_Init(&argc, &argv);
...
uint32_t A_recv_colsize = (n+1) / p;
uint32_t * recv = (uint32_t *)malloc((n + 1) * sizeof(uint32_t));
t1 = MPI_Wtime();

#pragma omp master
{
  MPI_Scatter(A_csc_col, A_recv_colsize, MPI_UINT32_T, recv, A_recv_colsize, MPI_UINT32_T, 0, MPI_COMM_WORLD);
}
...
*/
   ...
   uint32_t partial_C_csc[p];
   MPI_Gather(partial_C_csc, 1, MPI_UINT32_T, partial_C_csc, 1, MPI_UINT32_T,0, MPI_COMM_WORLD);
...
/*
  accum = (double)(( stop.tv_sec - start.tv_sec )
             + ( stop.tv_nsec - start.tv_nsec )
               / BILLION);
  printf( "BMM_MPI took %lf seconds to execute.\n", accum );
*/

printf( "BMM_MPI took %f seconds to execute\n", t2 - t1 );
...
/*
 do {
      printf("Enter the number of threads for OpenMP :" );
      scanf("%d", &NUM_THREADS);
  } while (NUM_THREADS <= 0);
*/
